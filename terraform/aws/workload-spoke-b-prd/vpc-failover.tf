locals {
  ...
  vpc_workload_spoke_b_tags_failover = {}
}
...
module "vpc_failover" {
  ...
  cidr = var.vpc_cidr_infrastructure.workload_spoke_b_prd_failover

  azs                 = local.azs_failover
  private_subnets     = local.vpc_workload_spoke_b_private_subnets_failover
  public_subnets      = []
  database_subnets    = []
  elasticache_subnets = []
  redshift_subnets    = []
  intra_subnets       = local.vpc_workload_spoke_b_intra_subnets_failover
  ...
  dhcp_options_domain_name_servers = [replace(var.vpc_cidr_infrastructure.workload_spoke_b_prd_failover, "0/16", "2")]
  dhcp_options_ntp_servers         = var.ntp_servers

  vpc_tags = local.vpc_workload_spoke_b_tags_failover
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "vpc_workload_spoke_b_to_tgw_failover" {
  provider = aws.workload_spoke_b_prd_failover
