module "sg_central_ingress_main_primary" {
  source    = "terraform-aws-modules/security-group/aws"
  version   = "5.3.0"
  providers = { aws = aws.networking_prd }

  name        = "${local.resource_name_stub_primary}-${var.this_slug}-central-ingress-main-sg"
  description = "${local.resource_name_stub_primary}-${var.this_slug}-central-ingress-main-sg"
  ...
  tags = { Name = "${local.resource_name_stub_primary}-${var.this_slug}-central-ingress-main-sg" }
}
