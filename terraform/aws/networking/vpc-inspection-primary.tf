...
#   private_subnet_names = [for i in range(6) : "${format("%s-pvt-", "${local.resource_name_stub_primary}-vpc-inspection-primary")}${i}"]
#   private_subnets = (
#     var.availability_zones_num_used == 6 ? cidrsubnets(var.vpc_cidr_infrastructure.inspection_primary, 4, 4, 4, 4, 4, 4) :
#     var.availability_zones_num_used == 5 ? cidrsubnets(var.vpc_cidr_infrastructure.inspection_primary, 4, 4, 4, 4, 4) :
#     var.availability_zones_num_used == 4 ? cidrsubnets(var.vpc_cidr_infrastructure.inspection_primary, 2, 2, 2, 2) :
#     var.availability_zones_num_used == 3 ? cidrsubnets(var.vpc_cidr_infrastructure.inspection_primary, 2, 2, 2) :
#     cidrsubnets(var.vpc_cidr_infrastructure.inspection_primary, 1, 1)
#   )
