locals {
  vpc_workload_spoke_a_cidrsubnets_primary = (
    var.azs_used == 4 ? cidrsubnets(var.vpc_cidr_infrastructure.workload_spoke_a_prd_primary, 3, 3, 3, 3, 12, 12, 12, 12) :
    var.azs_used == 3 ? cidrsubnets(var.vpc_cidr_infrastructure.workload_spoke_a_prd_primary, 2, 2, 2, 12, 12, 12) :
    var.azs_used == 2 ? cidrsubnets(var.vpc_cidr_infrastructure.workload_spoke_a_prd_primary, 2, 2, 12, 12) :
    null
  )
...
resource "aws_ec2_transit_gateway_route" "post_inspection_workload_spoke_a_primary_to_failover" {
  provider = aws.networking_prd
